        ...
        for page in doc:
            ...
            for img in images:
                ...
                try:
                    ...
                    if pix.n > 4:  # Si tiene canal alfa, convertir a RGB
                    ...
                    print(f"Error procesando imagen en xref {xref}: {e}")

        if images_found:
            doc.save(temp_output, garbage=4, deflate=True)
        else:
            print("No se encontraron imágenes en el PDF. Se omite la compresión de imágenes.")
...
def split_pdf(input_path, output_folder):
    """
    Divide un PDF en 2 partes con la misma cantidad de páginas.
    """
...
def process_file(file_path, threshold=1*1024*1024):
    """
    Procesa archivos PDF e imágenes para comprimirlos o dividirlos.
    """
    ...
    if ext in ['.jpg', '.jpeg', '.png']:
        ...
        if size_after > threshold:
            print(f"'{compressed_pdf_path}' aún pesa {size_after//1024} KB. Dividiendo...")
            ...
            print(f"Partes finales: {part1_path} ({os.path.getsize(part1_path)//1024} KB), {part2_path} ({os.path.getsize(part2_path)//1024} KB)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        root = tk.Tk()
        root.withdraw()
        files = filedialog.askopenfilenames(title="Selecciona archivos para comprimir (PDF/Imágenes)", filetypes=[("PDF e Imágenes", "*.pdf;*.jpg;*.jpeg;*.png")])
    for file in files:
